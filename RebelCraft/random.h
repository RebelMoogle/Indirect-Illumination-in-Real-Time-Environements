...
__host__ __inline__ optix::uint4 random4u()
{
  return optix::make_uint4(random1u(), random1u(), random1u(), random1u());
}
...
__device__ __inline__ optix::float2 rnd_from_uint2( optix::uint2& prev )
{
  return optix::make_float2(rnd(prev.x), rnd(prev.y));
}

__device__ __inline__ optix::float2 rnd_from_uint2( unsigned int& prev1, unsigned int& prev2 )
{
  return optix::make_float2(rnd(prev1), rnd(prev2));
}